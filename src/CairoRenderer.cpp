...
void CairoRenderer::renderToPDF(const LevelScheme& scheme, const std::string& filename){
	...
	//normalize energy to y scale:
	double maxEnergy = 0.0;
	for(const auto& l : levels){
		if(l.energy > maxEnergy) maxEnergy = l.energy;
	}
	double scaleY = (HEIGHT - 2 * MARGIN) / (maxEnergy + 100); //avoids clipping
	...
	for(const auto& lvl : levels){
		...
		// label << lvl.label << " (" << lvl.spinParity << "), " << lvl.energy << " keV";
		// drawText(cr, x2+TEXT_OFFSET, y+4, label.str());
